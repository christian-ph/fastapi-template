...
def test_create_user_success(client, mock_user_repo):
    ...
    user_input = {"email": "test@example.com", "full_name": "Test User", "password": "strongpassword"}
...
def test_read_user_success(client, mock_user_repo, mock_db):
    ...
    user_id = 99
    expected_entity = UserEntity(
        id=user_id, email="read@example.com", full_name="Reader", hashed_password="hash", is_active=True
    )
...
def test_update_user_success(client, mock_user_repo):
    ...
    existing_entity = UserEntity(
        id=user_id, email="original@example.com", full_name="Original", hashed_password="hash", is_active=True
    )

    updated_entity = UserEntity(
        id=user_id, email="original@example.com", full_name="Updated Name", hashed_password="hash", is_active=True
    )
